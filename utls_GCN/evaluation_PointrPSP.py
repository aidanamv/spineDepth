...
for fold in range(0,9):
    ...
    rgbds = np.load(dir+"/rgbd.npz")["arr_0"]
    ...
    for el,file in enumerate(files):
        ...
        red_cloud1 = data[red_points_mask1,:]
        red_cloud2 = data[red_points_mask2,:]
        red_cloud3 = data[red_points_mask3,:]
        red_cloud4 = data[red_points_mask4,:]

        ep1 = np.mean(red_cloud1.squeeze(0), axis=0)
        ep2 = np.mean(red_cloud3.squeeze(0), axis=0)
        mp1 = np.mean(red_cloud2.squeeze(0), axis=0)
        mp2 = np.mean(red_cloud4.squeeze(0), axis=0)

        direction_vector1 = mp1 - ep1
        direction_vector2 = mp2 - ep2
        if np.linalg.norm(direction_vector1)!=0 and np.linalg.norm(direction_vector2)!=0:
            ...
            # Compute the intersection between the two trimesh objects
            ...
            signed_distances1 = scene1.compute_signed_distance(query_point1).numpy()
            ...
            signed_distances2 = scene2.compute_signed_distance(query_point2).numpy()
            min_cylinder1 = np.min(signed_distances1)
            min_cylinder2 = np.min(signed_distances2)
            ind1 = np.argmin(signed_distances1)
            ind2 = np.argmin(signed_distances2)
...
df.to_csv("../evaluations/pointr_2048.csv", index=False)
print(df.describe())
